                        ...
                        @forelse($complaints as $complaint)
                            <tr>
                                ...
                                <td class="text-center">
                                    ...
                                    @else
                                        <span
                                            class="fw-bold">{{ $complaint->priority_score ? number_format((float) $complaint->priority_score, 4) : '—' }}</span>
                                    @endif
                                </td>
                                <td class="text-center">
                                    {{-- Tombol Detail --}}
                                    <button type="button" class="btn btn-sm btn-outline-info me-1" data-bs-toggle="modal"
                                        data-bs-target="#detailModal-{{ $complaint->id }}" title="Lihat Detail">
                                        <i class="bi bi-eye"></i>
                                    </button>
                                    @if($complaint->status === 'pending')
                                        {{-- Tombol Verifikasi: Terima / Tolak --}}
                                        <button type="button" class="btn btn-sm btn-success" data-bs-toggle="modal"
                                            data-bs-target="#approveModal-{{ $complaint->id }}" title="Terima Pengaduan">
                                            <i class="bi bi-check-lg"></i> Terima
                                        </button>
                                        <button type="button" class="btn btn-sm btn-danger ms-1" data-bs-toggle="modal"
                                            data-bs-target="#rejectModal-{{ $complaint->id }}" title="Tolak Pengaduan">
                                            <i class="bi bi-x-lg"></i> Tolak
                                        </button>
                                    @elseif($complaint->status === 'approved')
                                        <button type="button" class="btn btn-sm btn-primary" data-bs-toggle="modal"
                                            data-bs-target="#statusModal-{{ $complaint->id }}" title="Ubah ke Diproses">
                                            <i class="bi bi-arrow-right-circle"></i> Proses
                                        </button>
                                    @elseif($complaint->status === 'on_progress')
                                        <button type="button" class="btn btn-sm btn-success" data-bs-toggle="modal"
                                            data-bs-target="#statusModal-{{ $complaint->id }}" title="Tandai Selesai">
                                ...
                                <div class="modal-dialog">
                                    <div class="modal-content">
                                        <div class="modal-header border-0 pb-0">
                                            <h5 class="modal-title fw-bold"><i class="bi bi-eye me-2"></i>Detail Pengaduan
                                                {{ $complaint->tracking_code }}</h5>
                                        ...
                                        <div class="modal-body py-4">
                                            ...
                                            @if($complaint->evidence_path)
                                                <div class="mb-3">
                                                    ...
                                                    <div class="text-center">
                                                        ...
                                                        @if($base64Image)
                                                            <img src="{{ $base64Image }}" alt="Bukti Foto"
                                                                class="img-fluid rounded-3 border"
                                                                style="max-height: 300px; object-fit: contain;">
                                                        @else
                                                            <span class="text-muted small"><i class="bi bi-image"></i> Gambar tidak
                                                                tersedia/gagal dimuat.</span>
                                                        @endif
                                                    </div>
                                                </div>
                                            @endif
                                            @if($complaint->resolution_note)
                                                <div class="mb-3">
                                                    ...
                                                    <div class="bg-success bg-opacity-10 p-3 rounded-3 text-dark border-start border-4 border-success"
                                                        style="font-size: 0.85rem">{{ $complaint->resolution_note }}</div>
                                                </div>
                                            @endif
                                            @if($complaint->rejection_reason)
                                                <div class="mb-3">
                                                    <div class="small fw-bold text-muted mb-1">Alasan Penolakan</div>
                                                    <div class="bg-danger bg-opacity-10 p-3 rounded-3 text-dark border-start border-4 border-danger"
                                                        style="font-size: 0.85rem">{{ $complaint->rejection_reason }}</div>
                                                </div>
                                            @endif
                                        </div>
                                            ...
                                            <div class="modal-content">
                                                <div class="modal-header border-0 pb-0">
                                                    <h5 class="modal-title fw-bold text-success"><i
                                                            class="bi bi-whatsapp me-2"></i>Kirim Notifikasi WhatsApp?</h5>
                                                    <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                                                </div>
                                                <div class="modal-body py-4">
                                                    <div class="mb-3">
                                                    ...
                                                    <div class="mb-2">
                                                        ...
                                                        <div class="p-3 bg-light rounded text-dark"
                                                            style="font-size: 0.85rem; white-space: pre-wrap;">Halo Bapak/Ibu,
                                                            Pengaduan Anda untuk siswa *{{ $complaint->student->name ?? '-' }}*
                                                            dengan nomor tiket *{{ $complaint->tracking_code }}* telah *SELESAI*
                                                            kami proses dan tindak lanjuti.

                                                            Catatan Penyelesaian:
                                                            {{ $complaint->resolution_note ?? '-' }}

                                                            Terima kasih atas laporan Anda. Silakan cek aplikasi untuk detail
                                                            selengkapnya.
                                                        </div>
                                                    </div>
                                                </div>
                                                <div class="modal-footer border-0">
                                                    ...
                                                    <button type="submit" class="btn btn-success px-4"><i
                                                            class="bi bi-send-fill me-1"></i>Kirim WhatsApp</button>
                                                </div>
                                            </div>
                                        </form>
                                    </div>
                                    ...
                                    <div class="modal-dialog">
                                        <form action="{{ route('admin.complaints.verify', $complaint->id) }}" method="POST">
                                            ...
                                            <div class="modal-content">
                                                <div class="modal-header border-0 pb-0">
                                                    <h5 class="modal-title fw-bold text-success"><i
                                                            class="bi bi-check-circle me-2"></i>Terima Pengaduan</h5>
                                                    <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                                                </div>
                                                <div class="modal-body py-4">
                                                    <div class="alert alert-success bg-success bg-opacity-10 border-0 small">
                                                        Pengaduan <strong>{{ $complaint->tracking_code }}</strong> akan
                                                        diverifikasi.
                                                        Silakan beri nilai pada kondisi pengaduan berdasarkan kriteria AHP berikut
                                                        untuk penentuan prioritas secara otomatis.
                                                    </div>
                                                    ...
                                                    <div class="mb-4">
                                                        ...
                                                        @foreach($criteria as $criterion)
                                                            <div class="mb-3">
                                                                <label class="form-label fw-semibold small mb-1">{{ $criterion->name }}
                                                                    ({{ $criterion->code }}) <span class="text-danger">*</span></label>
                                                                <div class="small text-muted mb-2" style="font-size: 0.7rem;">
                                                                    {{ $criterion->description }}</div>
                                                                <select name="criteria[{{ $criterion->id }}]"
                                                    ...
                                                    <div class="text-muted small border-top pt-3">
                                                        ...
                                                        Sistem akan mengalikan nilai yang Anda berikan dengan bobot kriteria AHP
                                                        untuk menentukan prioritas akhir.
                                                    </div>
                                                </div>
                                                <div class="modal-footer border-0">
                                                    <button type="button" class="btn btn-light"
                                                        data-bs-dismiss="modal">Batal</button>
                                                    <button type="submit" class="btn btn-success px-4"><i
                                                            class="bi bi-check-lg me-1"></i>Terima & Proses AHP</button>
                                                </div>
                                            </div>
                                        </form>
                                    </div>
                                            ...
                                            <div class="modal-content">
                                                <div class="modal-header border-0 pb-0">
                                                    <h5 class="modal-title fw-bold text-danger"><i
                                                            class="bi bi-x-circle me-2"></i>Tolak Pengaduan</h5>
                                                    <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                                                </div>
                                                <div class="modal-body py-4">
                                                    <div class="alert alert-warning bg-warning bg-opacity-10 border-0 small mb-3">
                                                        Anda akan menolak pengaduan
                                                        <strong>{{ $complaint->tracking_code }}</strong>. Orang tua akan menerima
                                                    ...
                                                    <div class="mb-3">
                                                        ...
                                                        <textarea name="rejection_reason" class="form-control" rows="4"
                                                            placeholder="Masukkan alasan penolakan yang jelas untuk orang tua..."
                                                            required></textarea>
                                                    </div>
                                                </div>
                                                <div class="modal-footer border-0">
                                                    ...
                                                    <button type="submit" class="btn btn-danger px-4"><i
                                                            class="bi bi-x-lg me-1"></i>Tolak Pengaduan</button>
                                                </div>
                                            </div>
                                        </form>
                                    </div>
                                            ...
                                            <div class="modal-content">
                                                <div class="modal-header border-0 pb-0">
                                                    <h5 class="modal-title fw-bold"><i
                                                            class="bi bi-arrow-right-circle me-2"></i>Update Status Pengaduan</h5>
                                                    <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                                                </div>
                                                <div class="modal-body py-4">
                                                    <p class="text-muted small">Ubah status
                                                        <strong>{{ $complaint->tracking_code }}</strong> menjadi <strong
                                                            class="text-primary">{{ $nextLabel }}</strong>.</p>
                                                    @if($nextStatus === 'resolved')
                                                        <div class="mb-3">
                                                            <label class="form-label fw-semibold small">Catatan Penyelesaian
                                                                (Opsional)</label>
                                                            <textarea name="resolution_note" class="form-control" rows="3"
                                                                placeholder="Tuliskan catatan hasil penyelesaian pengaduan..."></textarea>
                                                        </div>
                                                    @endif
                                                </div>
                                                <div class="modal-footer border-0">
                                                    ...
                                                    <button type="submit" class="btn btn-primary-custom px-4"><i
                                                            class="bi bi-check-lg me-1"></i>Konfirmasi</button>
                                                </div>
                                            </div>
                                        </form>
                                    </div>
